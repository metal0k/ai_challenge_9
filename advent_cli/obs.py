...
def verify_capture(
    ...
    Кадр берётся НЕ один раз, и вот почему.

    2026-09-07 проверка отвергла исправную сцену: запись сорвалась на пустом
    кадре, а замер на том же окне через минуту дал долю непустых пикселей
    0.1527 при пороге 0.005 — в тридцать раз выше. **Причину воспроизвести
    не удалось.** Две версии проверены живыми замерами и ОТВЕРГНУТЫ:

    - «источнику не хватило времени на первый кадр» — повтор всей
      последовательности record() даёт 0.1527 уже на первой попытке, без
      всякого прогрева;
    - «OBS остался на мёртвом HWND, потому что строка окна не изменилась» —
      окно закрыто и создано заново с тем же заголовком, перепривязка
      сработала, первый кадр 0.0711.

    Отсюда решение: не подгонять объяснение, а убрать целый КЛАСС отказов.
    Одиночный мгновенный снимок — хрупкая конструкция по построению: он даёт
    дорогой отказ по одному наблюдению, и `ensure_scene()` это уже признаёт
    своим `time.sleep(1.0)`. Ретраи обобщают ту же оговорку.

    Ложное срабатывание стоит ровно столько же, сколько пропущенный чёрный
    кадр, — сорванного дубля. Поэтому именно ретраи, а не ослабленный порог:
    порог отличает чёрное от нечёрного, ретраи отличают «пусто» от «рано».
    Пропустить настоящий чёрный кадр это не может — непустой кадр всё так же
    обязателен, просто спрашиваем несколько раз.

    `sleep` параметром — чтобы тест не ждал секунды по-настоящему.
    """
    ...
    for attempt in range(attempts):
        ...
        try:
            ...
            # Ошибка запроса тоже может быть «рано»: источник в момент
            # перепривязки способен ответить отказом. Запоминаем и пробуем
            # ещё, но если так и не получится — скажем именно про неё.
            last_error = exc
            continue
        last_error = None
        if _has_content(data, min_bright_ratio):
            return

    if last_error is not None:
        raise AdventError(f"Не удалось снять пробный кадр источника: {last_error}") from last_error
